...
@ revision 2.1
    ...
    .equ    USED_GAME, GAME_A3UJ        @ CHOOSE YOUR GAME

    .equ    FRAME_LENGTH_5734, 0x60
    .equ    FRAME_LENGTH_7884, 0x84     @ THIS MODE IS NOT SUPPORTED BY THIS ENGINE BECAUSE IT DOESNT USE AN 8 ALIGNED BUFFER LENGTH
    ...
    .equ    BUFFER_IRAM_A3U, 0x03000000
...
@#######################################
@*********** GAME CONFIGS **************
@ add the games name above to the ASM .equ-s before creating new configs
@#######################################
...
.endif
@*********** IF MOTHER 3
.if USED_GAME==GAME_A3UJ

    .equ    hq_buffer, BUFFER_IRAM_A3U
    .equ    ALLOW_PAUSE, 0
    .equ    DMA_FIX, 0  @ DMA_FIX == 1 for m4a_hq_mixer_dma_fix.bin
    .equ    ENABLE_DECOMPRESSION, 0
    .equ    PREVENT_CLIP, 1
.endif
@***********
...
clear_buffer:
    /* Incase reverb is disabled the buffer gets set to zero */
...
adsr_decay_check:
        ...
        CMP R5, R0
        BHI channel_vol_calc            @ sample didnt decay yet --> branch
        /* sustain handler */
...
adsr_attack_check:
        ...
        BNE channel_vol_calc            @ if it isnt in attack attack phase, it has to be in sustain (no adsr change needed) --> branch
...
mixing_arm_setup:
         ...
         *  path 1: fast mixing, but doesnt supports loop or stop
...
BEQ uncached_mixing_load_skip           @ skip the mixing load if it isnt required
...
BXNE    LR                      @ if it loops return to mixing function, if it doesnt go on and end mixing
...
BNE fixed_mixing_loop               @ if the end wasnt reached yet, repeat the loop
...
BNE setup_compressed_mixing_frequency   @ skip the setup procedure if its running in compressed mode already
...
BEQ determine_compression           @ if Reverse Mode isnt enabled we can directly check if the sample has to get decoded
...
BEQ compressed_mixing_reverse_load_skip @ skip sample loading if we dont need to load new samples from ROM
